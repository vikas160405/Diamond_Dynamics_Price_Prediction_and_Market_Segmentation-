...
<style>
    ...
    .diamond-header {
        ...
        margin-bottom: 2rem; border: 1px solid #e94560;
    ...
    .stButton>button {
        ...
        padding: 0.6rem 2rem; width: 100%; transition: all 0.3s ease;
    }
...
CLUSTER_ICONS = {
    'Premium Heavy Diamonds':      ('💎', '#e74c3c', 'cluster-premium'),
...
<div class="diamond-header">
    ...
    <p>Price Prediction & Market Segmentation</p>
</div>
""", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    carat = st.slider("Carat Weight", min_value=0.2, max_value=5.0, value=1.0, step=0.01)
    x     = st.number_input("Length - x (mm)", min_value=0.1, max_value=15.0, value=6.0, step=0.1)
    y     = st.number_input("Width  - y (mm)", min_value=0.1, max_value=15.0, value=6.0, step=0.1)
    z     = st.number_input("Depth  - z (mm)", min_value=0.1, max_value=10.0, value=3.7, step=0.1)
    depth_val = st.slider("Depth %", min_value=40.0, max_value=80.0, value=62.0, step=0.1)
    table_val = st.slider("Table %", min_value=43.0, max_value=100.0, value=57.0, step=0.5)
...
def prepare_features(carat, cut, color, clarity, depth_val, table_val, x, y, z):
    ...
    Order: ['carat','cut','color','clarity','depth','table','volume','dimension_ratio']
    """
    volume    = x * y * z
...
def prepare_cluster_features(carat, cut, color, clarity, depth_val, table_val, x, y, z):
    ...
    Order: ['carat','cut','color','clarity','depth','table','volume']
    """
    volume      = x * y * z
    cut_enc     = CUT_MAP.get(cut, 2)
    color_enc   = COLOR_MAP.get(color, 3)
...
with tab1:
    ...
    with col_info:
        ...
        with metrics_col1:
            st.markdown(f'<div class="metric-tile"><div class="val">{carat}</div><div class="lbl">Carats</div></div>', unsafe_allow_html=True)
        with metrics_col2:
            st.markdown(f'<div class="metric-tile"><div class="val">{cut}</div><div class="lbl">Cut</div></div>', unsafe_allow_html=True)
        with metrics_col3:
            st.markdown(f'<div class="metric-tile"><div class="val">{color}</div><div class="lbl">Color</div></div>', unsafe_allow_html=True)

        st.markdown("<br>", unsafe_allow_html=True)
        mc1, mc2, mc3 = st.columns(3)
        with mc1:
            st.markdown(f'<div class="metric-tile"><div class="val">{clarity}</div><div class="lbl">Clarity</div></div>', unsafe_allow_html=True)
        with mc2:
            volume_display = round(x * y * z, 2)
            st.markdown(f'<div class="metric-tile"><div class="val">{volume_display}</div><div class="lbl">Volume (mm³)</div></div>', unsafe_allow_html=True)
        with mc3:
            dim_r = round((x + y) / (2 * z), 2) if z > 0 else 0
            st.markdown(f'<div class="metric-tile"><div class="val">{dim_r}</div><div class="lbl">Dim Ratio</div></div>', unsafe_allow_html=True)
    ...
    with col_predict:
        ...
        if st.button("🔮 Predict Price", key="btn_price"):
            ...
            if models['regressor'] is not None:
                # ✅ FIX: scale first, then predict on scaled features
                ...
                # Fallback formula if model not yet trained
            ...
            elif pred_inr < 300000:    tier, tip = "Mid-range","💡 Popular for engagement rings"
            elif pred_inr < 1000000:   tier, tip = "Premium",  "💡 Investment-grade diamond"
...
with tab2:
    ...
    with col_desc:
        ...
        st.markdown('<div class="card">', unsafe_allow_html=True)
        st.markdown("#### ⚙️ Clustering Method")
        st.markdown("""
...
with tab3:
    ...
    pipeline_steps = [
        ("1️⃣", "Data Preprocessing",   "Null handling, zero-value imputation, outlier removal (IQR)"),
        ("2️⃣", "EDA",                   "Distribution plots, boxplots, heatmap, pairplot"),
        ("3️⃣", "Feature Engineering",   "Volume, Price/Carat, Dimension Ratio, Carat Category, INR conversion"),
        ("4️⃣", "Encoding",              "Ordinal Encoding for cut, color, clarity"),
        ("5️⃣", "Feature Selection",     "VIF analysis, correlation matrix"),
        ("6️⃣", "Regression Models",     "Linear Reg, DT, RF, XGBoost, KNN + ANN (Keras)"),
        ("7️⃣", "Clustering",            "K-Means (k=3) via Elbow + Silhouette, PCA visualization"),
        ("8️⃣", "Deployment",            "Streamlit app with real-time price & cluster prediction"),
    ]
    cols = st.columns(4)
    for i, (num, title, desc) in enumerate(pipeline_steps):
        with cols[i % 4]:
            st.markdown(f"""
            <div style="background:#21262d;border-radius:10px;padding:1rem;
                        border:1px solid #30363d;margin-bottom:1rem;text-align:center;">
                <div style="font-size:1.8rem">{num}</div>
                <div style="color:#58a6ff;font-weight:bold;font-size:0.9rem">{title}</div>
                <div style="color:#8b949e;font-size:0.78rem;margin-top:0.3rem">{desc}</div>
            </div>
            """, unsafe_allow_html=True)
...
st.markdown("---")
st.markdown(
    "<center><p style='color:#8b949e;font-size:0.85rem'>💎 Diamond Dynamics | Built with Streamlit</p></center>",
    unsafe_allow_html=True
)